...
## Executive Summary
The bar chart shows the predicted number of optimal bays against the average actual number of bays for each category in September, based on the data for 2024 (top graph) and for 2022-2024 (bottom graph). Firstly, it is important to note that we set a maximum for all categories of 3 bays. Therefore, where we see the recommendation to decrease the number of bays for the home category, we ignore this because we set a limit of 3, and the reason home was originally above 3 was due to extra space being available at the time.
...
We will begin with ladies dresses. The bottom line graph shows the trend for quantity of dresses sold throughout the 3 and a half years. It shows more dresses are sold in the summer and less in the winter, as we would expect. The top line graph is of more interest to us here. It shows the percentage of floor space the dresses occupy as well as the percentage of the shop's revenue dresses create over time. We can see that for the last 3 winters the percentage of space occupied by dresses is consistently higher than the percentage of revenue they generate for the shop. We should also note that in the winter of 2024 the percentage of revenue generated by dresses is close to 0% on numerous occasions, with its lowest being 0.31%, however the percentage of space is roughly 4.5%. This implies we should look to decrease the number of dresses on display throughout the winter even further and put something else in their place instead, as is suggested by the optimisation graph above.
...
- I noticed for the weeks 14/01/2024 to 28/01/2024 that data values were 0 for all columns. After discussion with the shop's manager this was due to the shop being closed for a refurbish and these 3 weeks were removed from the data set since they do not show a true representation of the data.
- I created two new columns, 'modified month' and 'modified year', to be used as the months and years for graphs and calculations, since our dates are for the date the week commences. These new columns account for dates where maybe there are only 1 or 2 days in that week which are in a particular month or year. For example, if a week begins on 31st december 2024 we would give this a modified month of January and a modified year of 2025, since 6 of the 7 days fall into January 2025.
- After discussion with the shop's manager the categories electrical, furniture and non clothing promotion are not sold in the shop so any of the sales for these were treated as human error at the till and moved to the categories jewellery, books and kids non clothing, respectively. Therefore, all other column values (for example the percentage of the revenue each category holds for that week) were updated too before analysis began.

A Python Jupyter notebook with more explanation on these changes can be found [here](https://github.com/AaronMo13/Charity-Shop-Floor-Plan-Optimisation/blob/main/Charity%20Shop%20Data%20Transform.ipynb). 
